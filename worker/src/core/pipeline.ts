...
export async function runTranslateJob(
  ...
  const extracted = extract(job.content, job.glossary, {
    sourceLang: job.source, stripSdhEnabled: job.stripSdhEnabled, sceneChangeSeconds: job.sceneChangeSeconds,
  });
  if (!extracted.success) {
    return {
      success: false, resolved_source_lang: job.source, sdh_removed: extracted.sdh_removed,
      cues: [], approx_splits: [], missing_count: 0, missing_cues: [],
    };
  }
  ...
  return { success: true, resolved_source_lang: job.source, sdh_removed: extracted.sdh_removed, ...merged };
}
